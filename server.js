...
app.post('/send-email', (req, res) => {
  ...
  // Configuração do transportador para SMTP Locaweb
  const transporter = nodemailer.createTransport({
    host: "email-ssl.com.br", // Host fornecido pela Locaweb para SMTP
    port: 587,             // Porta para conexão STARTTLS; para SSL direta use 465 e secure: true
    secure: false,         // false para usar STARTTLS
    auth: {
      user: 'leads.site@sticktape.com.br', // Seu e-mail configurado na Locaweb
      pass: 'Sticktape@222300'        // A senha fornecida no painel de controle da Locaweb
    },
    tls: {
      // Permite conexões mesmo com certificados não verificados (opcional)
  ...
  const mailOptions = {
    from: 'leads.site@sticktape.com.br', // É recomendável usar o mesmo e-mail autenticado
    replyTo: email,                  // Define o e-mail do remetente para respostas
    to: 'jefferson@sticktape.com.br',   // Destinatário
...
app.post("/orcamentos", async (req, res) => {
  ...
  db.query(
    query,
    [produtosJSON, nome_cliente, email, telefone],
    (err, result) => {
      if (err) {
        console.error(err);
...
app.put("/orcamentos/:id", verifyToken, async (req, res) => {
  ...
  db.query(
    query,
    [produtosJSON, nome_cliente, email, telefone, id],
    (err, result) => {
      if (err) {
        console.error(err);
        return res
          .status(500)
          .json({ message: "Erro ao atualizar orçamento." });
      }
...
// Deletar um orçamento (rota protegida)
...
app.post("/users", verifyToken, async (req, res) => {
  ...
  const query =
    "INSERT INTO users (name, email, pass, photo) VALUES (?, ?, ?, ?)";
...
app.put("/users/:id", verifyToken, async (req, res) => {
  ...
  const query = hashedPassword
    ? "UPDATE users SET name = ?, email = ?, pass = ?, photo = ? WHERE id = ?"
...
app.delete("/produtos/imagens", verifyToken, (req, res) => {
  ...
  const query =
    "DELETE FROM produto_imagens WHERE produto_id = ? AND imagem = ?";
...
app.get("/produtos/:id", verifyToken, (req, res) => {
  ...
  const query = `
    SELECT 
      p.*, 
      GROUP_CONCAT(pi.imagem SEPARATOR ',') AS imagens
    FROM produto p
...
app.get("/produtos/:id/imagens", (req, res) => {
  ...
  db.query(query, [id], (err, results) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ message: "Erro ao buscar imagens do produto." });
    }
...
// Agora recebendo os campos "largura" e "comprimento"
app.post("/produtos", verifyToken, (req, res) => {
  ...
  const queryProduto = `
    INSERT INTO produto (nome, largura, comprimento, descricao, especificacoes, principais_aplicacoes, vantagens)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `;
  db.query(
    queryProduto,
    ...
    (err, result) => {
      ...
      const queryProdutoTipo =
        "INSERT INTO produto_tipo (produto_id, tipo_id) VALUES ?";
      db.query(queryProdutoTipo, [valuesTipos], (err2) => {
        if (err2) {
          console.error(err2);
          return res
            .status(500)
            .json({ message: "Erro ao associar tipos ao produto." });
        }
        if (imagens && imagens.length > 0) {
          const valuesImagens = imagens.map((img, index) => [
            produtoId,
            img,
            index + 1,
          ]);
          ...
          db.query(queryImagens, [valuesImagens], (err3) => {
            if (err3) {
              console.error(err3);
              return res
                .status(500)
                .json({ message: "Erro ao inserir imagens do produto." });
            }
            res.json({
    ...
    (err, result) => {
      ...
      db.query(queryDeleteAssoc, [id], (err2) => {
        if (err2) {
          ...
          return res
            .status(500)
            .json({ message: "Erro ao atualizar associações de tipos." });
        }
        if (tipos && tipos.length > 0) {
          const valuesTipos = tipos.map((tipoId) => [id, tipoId]);
          const queryInsertAssoc =
            "INSERT INTO produto_tipo (produto_id, tipo_id) VALUES ?";
          db.query(queryInsertAssoc, [valuesTipos], (err3) => {
            if (err3) {
              console.error(err3);
              return res
                .status(500)
                .json({
                  message: "Erro ao inserir novas associações de tipos.",
                });
            }
            if (imagens && imagens.length > 0) {
              const queryDeleteImagens =
              ...
              db.query(queryDeleteImagens, [id], (err4) => {
                if (err4) {
                  ...
                  return res
                    .status(500)
                    .json({ message: "Erro ao atualizar imagens do produto." });
                }
                const valuesImagens = imagens.map((img, index) => [
                  id,
                  img,
                ...
                const queryInsertImagens =
                  "INSERT INTO produto_imagens (produto_id, imagem, ordem) VALUES ?";
                db.query(queryInsertImagens, [valuesImagens], (err5) => {
                  if (err5) {
                    console.error(err5);
                    return res.status(500).json({
                      message: "Erro ao inserir novas imagens do produto.",
                    });
                  }
...
app.delete("/produtos/:id", verifyToken, (req, res) => {
  ...
  db.query(queryDeleteAssoc, [id], (err, result) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ message: "Erro ao deletar associações do produto." });
    }
...
app.get("/acessorios", (req, res) => {
  const query = "SELECT * FROM produto WHERE tipo = 'Acessório'";
  db.query(query, (err, results) => {
    if (err) {
      console.error(err);
...
app.post("/auth/login", (req, res) => {
  ...
  db.query(query, [email], async (err, results) => {
    ...
    const isMatch =
      password === user.pass || (await bcrypt.compare(password, user.pass));
    if (!isMatch) {
      return res.status(401).json({ message: "Email ou senha inválidos." });
    }
